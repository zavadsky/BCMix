...
void genRMD2(int32_t maxCode)
{
	int32_t nl[45] = { 0 };
	int32_t k[20] = { 0,1,30 };
	int32_t kmax = 3, Lmin = 3;
...
void encodeCodes2(uint32_t* codes, uint32_t codesN, uint8_t*& file, uint32_t& fileN)
{
...
void genRMD2_nl_nlk(int32_t maxCode)
{
	int32_t k[20] = { 0,1,30 };
	int32_t kmax = 3, Lmin = 3;
...
ai_state a_step1(ai_state prev, int32_t bit)
{
	state_max = 3;
...
void formTrmd_inner1_new2(int32_t L, ai_state state)
{
...
void formTrmd2_new()
{
	...
	for (int32_t a = 0; a < state_max; a++)
	{
		for (int32_t L = 0; L < 45; L++)
		{
			...
			formTrmd_inner1_new2(L, state);
		}
	}
}

void precalc2(uint32_t maxCode)
{
	genRMD2_nl_nlk(maxCode);
	formTrmd2_new();
}
...
int main(int argc, char** argv)
{
	...
	{
		int64_t maxCode = 0;
		for (int32_t i = 0; i < codesN; i++) { if (codes[i] > maxCode) { maxCode = codes[i]; } }
		cout << codesN << " " << maxCode << "--\n";

		writeCodes(string(argv[4]), nullptr, 0);
	...
	encodeCodes2(codes, codesN, file, fileN);
